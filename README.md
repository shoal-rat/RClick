...
- **No Xcode needed (recommended):** install the upstream release into `/Applications`, then run `rclick-agent/build-extension.sh`. It compiles the extension from this fork's patched sources with plain `swiftc` (Command Line Tools), grafts the binary into the release bundle, ad-hoc re-signs it, and re-registers the extension. The bundle's Developer-ID signature becomes ad-hoc — acceptable for an app that never runs (only its extension does).
- **Build the whole app from this fork** with Xcode 16+ — the source changes are already in place.

### 2. Fix: the menu appears in **every** folder

Upstream only asks Finder to observe `/Users/` and external volumes, so folders such as `/Applications`, `/opt`, or `/tmp` silently kept the plain context menu. The extension now observes `/`, which is what upstream's own "whole-disk observation" comment intended. (FileProvider-backed windows — iCloud Drive and iCloud-synced Desktop & Documents — still get no FinderSync menus from any extension; that is a macOS restriction.)

### 3. Fix: PermissionChecker no longer trips the same prompt
...
### 4. New: `rclick-agent/` — an invisible LaunchAgent replacing the main app
...
Everything below needs only the Xcode **Command Line Tools** — no full Xcode.

```sh
# 1. Extension: install the upstream v2.0.4 release
#    (https://github.com/wflixu/RClick/releases) into /Applications,
#    then rebuild its binary from this fork's patched sources:
cd rclick-agent
./build-extension.sh
